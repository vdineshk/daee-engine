  ...
  async scheduled(controller, env2, ctx) {
    ...
    ctx.waitUntil((async () => {
      try {
        ...
        try {
          const indexNowUrl = `https://api.indexnow.org/indexnow?url=${encodeURIComponent('https://dominion-observatory.sgdata.workers.dev/reports/' + reportDate)}&key=a1b2c3d4e5f6g7h8`;
...
`,
    ...
    if (url.pathname === "/sitemap.xml") {
      try {
        ...
        } catch(e) { /* reports table may not exist yet */ }
...
`;
    ...
    // IndexNow key file
    if (url.pathname === "/a1b2c3d4e5f6g7h8.txt") {
      return new Response("a1b2c3d4e5f6g7h8", {
        headers: { "Content-Type": "text/plain; charset=utf-8", "Cache-Control": "public, max-age=86400" }
      });
    }
