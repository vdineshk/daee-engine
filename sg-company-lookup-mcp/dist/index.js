...
// src/index.ts
var SERVICE_NAME = "sg-company-lookup-mcp";
var SERVICE_VERSION = "1.0.0";
var UPGRADE_URL = "https://daee-sg-company.vercel.app";
var FREE_TIER_DAILY_LIMIT = 5;
var FREE_TIER_DELAY_MS = 3e3;
function parseUen(uen) {
...
__name(parseUen, "parseUen");
var INDUSTRY_STATS = {
  overview: {
    total_live_entities: 83e4,
    data_as_of: "2024-Q4",
...
var TOOLS = [
  {
    name: "lookup_company",
    description: "Search Singapore companies by name or UEN, return registration details. Use this tool when you need to find or verify a Singapore registered business.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Directors and officers for a given UEN. Use this tool when you need to identify directors or key personnel of a Singapore company for KYC or due diligence.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Company registration/cessation trends by SSIC sector. Use this tool when you need market intelligence on business formation or closure trends in Singapore.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Active/struck-off/dissolved status check for any UEN. Use this tool when you need to verify whether a Singapore company is currently active.",
    inputSchema: {
      type: "object",
      properties: {
...
function toolLookupCompany(args) {
  ...
  if (isUen) {
    ...
    return {
      ...
      summary: uenInfo.valid ? `Valid UEN: ${uenInfo.uen} \u2014 ${uenInfo.entity_type}${uenInfo.registration_year ? `, registered ${uenInfo.registration_year}` : ""}` : `Invalid UEN format: ${query}`
    };
  }
  return {
    data: {
...
__name(toolLookupCompany, "toolLookupCompany");
function toolGetOfficers(args) {
  const uen = (args.uen || "").trim();
  ...
  return {
    ...
    summary: uenInfo.valid ? `UEN ${uen} (${uenInfo.entity_type}). Officer data requires BizFile+ (SGD 5.50). Entity type and registration year derived from UEN.` : `Invalid UEN: ${uen}. Cannot look up officers for invalid UEN.`
  };
}
__name(toolGetOfficers, "toolGetOfficers");
function toolGetIndustryStats(args) {
  const sector = (args.sector || "").trim().toUpperCase();
  const year = args.year;
  let filteredTrends = INDUSTRY_STATS.registration_trends;
  if (year) {
    const yearStr = String(year);
    const entry = filteredTrends[yearStr];
    if (entry) {
      filteredTrends = { [yearStr]: entry };
    } else {
  ...
  if (sector) {
    ...
    if (sectionCode && INDUSTRY_STATS.ssic_sections[sectionCode]) {
      sectorInfo = {
        ssic_section: sectionCode,
        name: INDUSTRY_STATS.ssic_sections[sectionCode],
  ...
  return {
    data: {
      ...
      ssic_sections: sector ? void 0 : INDUSTRY_STATS.ssic_sections,
      source: "ACRA Annual Report / Department of Statistics Singapore"
    },
    summary: sectorInfo ? `Singapore industry stats for ${sectorInfo.name} (Section ${sectorInfo.ssic_section}). ${sectorInfo.ranking ? `${sectorInfo.ranking.percentage}% of all entities.` : ""}` : `Singapore business overview: ~${INDUSTRY_STATS.overview.total_live_entities.toLocaleString()} live entities. Top sector: Wholesale & Retail Trade (18.2%).`
  };
}
__name(toolGetIndustryStats, "toolGetIndustryStats");
function toolCheckStatus(args) {
  ...
  return {
    data: {
      uen,
      uen_valid: uenInfo.valid,
      uen_analysis: uenInfo.valid ? uenInfo : void 0,
      live_status: {
    ...
    summary: uenInfo.valid ? `UEN ${uen} format is valid (${uenInfo.entity_type}). Live status check requires ACRA website or BizFile+ (SGD 5.50).` : `Invalid UEN format: ${uen}. Cannot check status.`
  };
}
__name(toolCheckStatus, "toolCheckStatus");
function buildMeta(tier, callsRemainingToday) {
  return {
    ...
    related_tools: {
      "sg-regulatory-data": "https://sg-regulatory-data-mcp.workers.dev",
  ...
  async fetch(request, env) {
    try {
      ...
      if (request.method === "GET" && path === "/health") {
        return jsonResponse({ status: "ok", service: SERVICE_NAME, version: SERVICE_VERSION, timestamp: (/* @__PURE__ */ new Date()).toISOString() });
      }
      ...
      if (request.method === "GET" && path === "/") {
        return jsonResponse({
          ...
          free_tier: "5 calls/day with 3s delay",
          paid_tier: "Unlimited. Authorization: Bearer daee_sk_xxxxx"
        });
      }
      return jsonResponse({ error: "Not found", available_endpoints: ["/", "/health", "/.well-known/mcp.json", "/mcp"] }, 404);
    } catch (error) {
      return jsonResponse({ error: "Internal server error", message: error instanceof Error ? error.message : String(error), service: SERVICE_NAME, timestamp: (/* @__PURE__ */ new Date()).toISOString() }, 500);
    }
  }
};
export {
